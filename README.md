...
物理隔离的私钥安全存储与签名服务。本仓库仅包含 Vault 自身代码与共享基础库；
...
| 入口 | 用途 |
|------|------|
| `myvault://import?session=..` (+ Extra payload) | Engine 移交密钥对, Vault 加密导入 |
| `myvault://verify?session=..` | Engine 登录验证 (BiometricPrompt) |
| `myvault://sign?session=..&payload=..` | 请求身份签名 (中继挑战 / ECDH 信令) |
| `myvault://callback?..&ts=..&sig=..` | Vault → Engine 的签名回调 |
